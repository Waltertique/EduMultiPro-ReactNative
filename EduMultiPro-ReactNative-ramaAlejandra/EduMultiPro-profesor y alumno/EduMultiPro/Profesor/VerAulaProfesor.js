...
export default function VerAulaProfesor() {
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      try {
        ...
        const resAnuncios = await apiFetch(`Aula/${id}`);
        ...
        for (const anuncio of anunciosData) {
          const resC = await apiFetch(`Anuncio/${anuncio.ID}`);
  ...
  const handleCrearAnuncio = async () => {
    try {
      ...
      // Recargar anuncios
      const anunciosRes = await apiFetch (`Anuncios/Aula/${id}`);
  ...
  const handleComentarioAnuncio = async (anuncioID, descripcion) => {
    try {
      ...
      const resC = await apiFetch(`Comentarios/Anuncio/${anuncioID}`);
    ...
    Alert.alert(
      ...
      [
        { text: "Cancelar", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              const res = await apiFetch(`Anuncios/${idAnuncio}`, {
                method: "DELETE",
              });
              const data = await res.json();
              Alert.alert("Éxito", data.mensaje);
  ...
  const handleModificarAnuncio = async () => {
    try {
      ...
      const res = await apiFetch(`Anuncios/${anuncioEditandoId}`, {
        method: "PUT",
        body: formData,
      });
                ...
                <View style={styles.anuncioContainer}>
                  <View style={styles.anuncioHeader}>
                    <View style={styles.anuncioUsuario}>
                      <Image
                        source={{
                          uri: `${STATIC_URL}/imagenes/${anuncio.RutaFoto || 'usuario.png'}`
                        }}
                  ...
                  {anuncio.Enlace_Anuncio && anuncio.Enlace_Anuncio.split(";").map((link, i) => (
                    <TouchableOpacity
                      key={i}
                      onPress={() => Linking.openURL(`imagenes/${link}`)}
                    >
      ...
      {comentarios.map(comentario => (
        <View key={comentario.ID} style={styles.comentarioItem}>
          <View style={styles.comentarioHeader}>
            <Image
              source={{
                uri: `${STATIC_URL}/imagenes/${comentario.RutaFoto || 'usuario.png'}`
              }}
